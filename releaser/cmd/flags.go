...
var BuildFlags = []cli.Flag{
	cli.StringFlag{
		Name:  BuildCmd + ", c",
		Usage: "The `CMD` to run during the building of the artifact. If no command is provided and a Makefile is present, releaser will run `make build` if `build` is a valid Make target, otherwise falling back to just `make`. If no `CMD` is provided and there is no Makefile, nothing will be done during the build phase.",
	},
	cli.StringFlag{
